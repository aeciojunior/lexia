  ...
  try {
    ...
    for (const auto of automations) {
      ...
      if (event === "process_created") {
        ...
        // Check movements that indicate status change
        const since = auto.last_run_at || new Date(now.getTime() - 2 * 60 * 60 * 1000).toISOString();
        const { data } = await supabase
      ...
      for (const item of filtered) {
        ...
        // Deduplicate: check if we already processed this item for this automation today
      ...
      await supabase
        ...
        .update({
          ...
          run_count: (auto.run_count || 0) + filtered.length,
        })
        .eq("id", auto.id);
    }
